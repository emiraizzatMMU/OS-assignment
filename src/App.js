...
import { Button, Collapse, Fade, MenuItem, Paper, TextField, Typography } from "@mui/material";
...
const App = props => {
	...
	const handleCalculate = () => {
		if (algoSelected == 0) {
			props.customNotification("Please select one scheduling algorithm", "error");
	...
	return (
		<Layout>
			<div className={classes.root}>
				<Typography className={classes.sectionTitle} style={{ marginBottom: "1rem" }}>
					OS Assignment 2110
				</Typography>
				<ul style={{ marginBottom: "3rem" }}>
					<li>
						<Typography className={classes.lecName}>Lecturer name: Ts. Dr. Ng Hu</Typography>
					</li>
					<li>
						<Typography className={classes.stuName}>1181302515: Azizi bin Mohamad Amin</Typography>
					</li>
					<li>
						<Typography className={classes.stuName}>1181302224: Adry Hilman bin Mohd Azman</Typography>
					</li>
					<li>
						<Typography className={classes.stuName}>
							1181302515: Muhammad Emir Aizzat bin Abdillah
						</Typography>
					</li>
				</ul>
				<TableContainer component={Paper}>
					<Table sx={{ minWidth: 650 }} aria-label="simple table">
						<TableHead style={{ backgroundColor: primaryLight, color: "white" }}>
							<TableRow>
					...
					<Fade in={isResult}>
						<div className={classes.resultContainer}>
							...
							<div className={classes.resultContainerRow}>
								<TitleDesc title="Total turnaround time" desc={result.turnaround.total} />
								<TitleDesc title="Average turnaround time" desc={result.turnaround.average} />
							</div>
							<div className={classes.resultContainerRow}>
								<TitleDesc title="Total waiting time" desc={result.waiting.total} />
								<TitleDesc title="Average waiting time" desc={result.waiting.average} />
							</div>
						</div>
					</Fade>
				) : null}
